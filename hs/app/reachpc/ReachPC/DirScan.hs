...
import System.Directory (createDirectoryIfMissing, doesFileExist, getModificationTime, listDirectory, pathIsSymbolicLink)
...
hashDirectory root = do
  ...
  where
    ...
    doScan cacheTree cacheMTime ignores (dir : dirStack) hashTree = do
      ...
      let pathTypes' =
            [ (p, isFile) | (p, isFile, isSymlink) <- pathTypes, not isSymlink, not $ isIgnored ignores' p
            ]
